...
class Test_wifi():
    ...
    def test_wifi_1():
        ...
        wlan0_list = wifi_set.Wifi_scan.wlan0_ssid_scan(wlan_ssid)
        print (wlan0_list)
        # 通过2G ssid筛选出对应的mac，然后判断字符串中是否有路由器2G的mac地址
        wlan1_list = wifi_set.Wifi_scan.wlan1_ssid_scan(wlan_ssid)
        print (wlan1_list)
    ...
    def test_wifi_2():
        ...
        # 通过5G ssid筛选出对应的mac，然后判断字符串中是否有路由器5G的mac地址
        con_status = wifi_set.Wifi_con.wifi_2g_wpa2_aes_connect(wlan_ssid, wlan_password)
        print(con_status)
        print("###############")
        if con_status == 1:
            linkrate = wifi_set.Linkrate_check.wifi_2g_linkrate()
    ...
    def test_wifi_3():
        ...
        wlan_password = wifi_set_conf.wlan_password
        # 通过5G ssid筛选出对应的mac，然后判断字符串中是否有路由器5G的mac地址
        con_status = wifi_set.Wifi_con.wifi_5g_wpa2_aes_connect(wlan_ssid, wlan_password)
        if con_status == 1:
            linkrate = wifi_set.Linkrate_check.wifi_5g_linkrate()
            print("xxxxxxxxxxxxxx")
            print(linkrate)
    ...
    def test_wifi_4():
        ...
        mac_2g = wifi_set_conf.mac_2g
        # 通过5G ssid筛选出对应的mac，然后判断字符串中是否有路由器5G的mac地址
        wlan0_list = wifi_set.Wifi_scan.wlan0_ssid_scan(wlan0_ssid)
        print (wlan0_list)
        # 通过2G ssid筛选出对应的mac，然后判断字符串中是否有路由器2G的mac地址
        wlan1_list = wifi_set.Wifi_scan.wlan1_ssid_scan(wlan1_ssid)
        print (wlan1_list)
        if wlan0_list != 0 and wlan1_list != 0:
    ...
    def test_wifi_6():
        wlan_ssid = wifi_set_conf.ssid_5g
        wlan_password = wifi_set_conf.wlan_password
        # 通过5G ssid筛选出对应的mac，然后判断字符串中是否有路由器5G的mac地址
        con_status = wifi_set.Wifi_con.wifi_5g_wpa2_aes_connect(wlan_ssid, wlan_password)
        if con_status == 1:
    ...
    def test_wifi_11():
        ...
        if wlan0_list != 0:
            if wlan0_list.find("%s" % mac_5g) == -1:
                # 通过5G ssid筛选出对应的mac，然后判断字符串中是否有路由器5G的mac地址
                con_status = wifi_set.Wifi_con.wifi_5g_wpa2_aes_auto_connect(wlan_ssid, wlan_password)
                if con_status == 1:
                    result = 1
                else:
                    result = 0
    ...
    def test_wifi_12():
        ...
        if wlan1_list != 0:
            if wlan1_list.find("%s" % mac_2g) == -1:
                # 通过2.4G ssid筛选出对应的mac，然后判断字符串中是否有路由器2.4G的mac地址
                con_status = wifi_set.Wifi_con.wifi_2g_wpa2_aes_auto_connect(wlan_ssid, wlan_password)
                if con_status == 1:
                    result = 1
                else:
                    result = 0
    ...
    def test_wifi_15():
        wlan_ssid = wifi_set_conf.ssid_2g_20b
        mac_5g = wifi_set_conf.mac_5g
        wlan_password = wifi_set_conf.wlan_password_24b
        # 通过5G ssid筛选出对应的mac，然后判断字符串中是否有路由器5G的mac地址
        wlan0_list = wifi_set.Wifi_scan.wlan0_ssid_scan(wlan_ssid)
        print (wlan0_list)
        if wlan0_list != 0:
    ...
    def test_wifi_17():
        wlan_ssid = wifi_set_conf.ssid_2g_1b
        mac_5g = wifi_set_conf.mac_5g
        wlan_password = wifi_set_conf.wlan_password_8b
        # 通过5G ssid筛选出对应的mac，然后判断字符串中是否有路由器5G的mac地址
        wlan0_list = wifi_set.Wifi_scan.wlan0_ssid_scan(wlan_ssid)
        print (wlan0_list)
        if wlan0_list != 0:
    ...
    def test_wifi_19():
        ...
        mac_5g = wifi_set_conf.mac_5g
        wlan_password = wifi_set_conf.wlan_password
        wlan_ssid_change = wifi_set_conf.ssid_2g_change
        mac_2g = wifi_set_conf.mac_2g
        ...
        if wlan0_list == 0 and wlan1_list == 0:
            ...
            wlan0_change_list = wifi_set.Wifi_scan.wlan0_ssid_scan(wlan_ssid_change)
            wlan1_change_list = wifi_set.Wifi_scan.wlan1_ssid_scan(wlan_ssid_change)
            ...
            if wlan0_change_list != 0 and wlan1_change_list != 0:
                ...
                if wlan0_change_list.find("%s" % mac_5g) != -1 and wlan1_change_list.find("%s" % mac_2g) != -1:
                    ...
                    con_status_5g = wifi_set.Wifi_con.wifi_5g_wpa2_aes_auto_connect(wlan_ssid, wlan_password)
                    con_status_2g = wifi_set.Wifi_con.wifi_2g_wpa2_aes_auto_connect(wlan_ssid, wlan_password)
                    con_status_5g_change = wifi_set.Wifi_con.wifi_5g_wpa2_aes_connect(wlan_ssid_change, wlan_password)
                    con_status_2g_change = wifi_set.Wifi_con.wifi_2g_wpa2_aes_connect(wlan_ssid_change, wlan_password)
    ...
    def test_wifi_20():
        ...
        if wlan0_list != 0 and wlan1_list != 0:
            ...
            if wlan0_list.find("%s" % mac_5g) != -1 and wlan1_list.find("%s" % mac_2g) != -1:
                ...
                con_status_5g = wifi_set.Wifi_con.wifi_5g_wpa2_aes_auto_connect(wlan_ssid, wlan_password)
                con_status_2g = wifi_set.Wifi_con.wifi_2g_wpa2_aes_auto_connect(wlan_ssid, wlan_password)
                con_status_5g_change = wifi_set.Wifi_con.wifi_5g_wpa2_aes_connect(wlan_ssid, wlan_password_change)
                con_status_2g_change = wifi_set.Wifi_con.wifi_2g_wpa2_aes_connect(wlan_ssid, wlan_password_change)
                print("连接状态：%s，%s，%s，%s" % (con_status_5g, con_status_2g, con_status_5g_change, con_status_2g_change))
                if con_status_5g == 0 and con_status_2g == 0 and con_status_5g_change == 1 and con_status_2g_change == 1:
                    result = 1
                else:
    ...
    def test_wifi_21():
        wlan_ssid = wifi_set_conf.ssid_2g
        con_status_5g = wifi_set.Wifi_con.wifi_5g_open_connect(wlan_ssid)
        con_status_2g = wifi_set.Wifi_con.wifi_2g_open_connect(wlan_ssid)
    ...
    def test_wifi_22():
        wlan_ssid = wifi_set_conf.ssid_2g
        wlan_password = wifi_set_conf.wlan_password
        con_status_2g_aes = wifi_set.Wifi_con.wifi_2g_wpa_aes_connect(wlan_ssid, wlan_password)
    ...
    def test_wifi_23():
        wlan_ssid = wifi_set_conf.ssid_2g
        wlan_password = wifi_set_conf.wlan_password
        con_status_2g_aes = wifi_set.Wifi_con.wifi_2g_wpa2_aes_connect(wlan_ssid, wlan_password)
        con_status_5g_aes = wifi_set.Wifi_con.wifi_5g_wpa2_aes_connect(wlan_ssid, wlan_password)
